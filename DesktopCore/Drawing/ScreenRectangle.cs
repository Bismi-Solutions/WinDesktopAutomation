...
namespace BismiSolutions.DesktopCore.Drawing
{
    internal class ScreenRectangle
    {
        ...
        internal ScreenRectangle(Rect rectangle)
        {
            ...
            //Set popup style
            int num1 = BismiSolutions.DesktopCore.WindowsAPI.NativeWindow.GetWindowLong(form.Handle, -20);
            BismiSolutions.DesktopCore.WindowsAPI.NativeWindow.SetWindowLong(form.Handle, -20, num1 | 0x80);

            //Set position
            BismiSolutions.DesktopCore.WindowsAPI.NativeWindow.SetWindowPos(form.Handle, new IntPtr(-1), Convert.ToInt32(rectangle.X), Convert.ToInt32(rectangle.Y),
                Convert.ToInt32(rectangle.Width), Convert.ToInt32(rectangle.Height), 0x10);
        }

        internal virtual void Show()
        {
            BismiSolutions.DesktopCore.WindowsAPI.NativeWindow.ShowWindow(form.Handle, 8);
        }
